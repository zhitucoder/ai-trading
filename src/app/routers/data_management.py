...
def data_status():
    ...
    ads = query("""
        SELECT
          ...
          (SELECT COUNT(*) FROM ads_stock_fund_trend) AS stock_fund_trend,
    ...
    def _run():
        ...
        try:
            ...
            """, (result['stock_latest'], result['stock_latest'],
                  ...
                  f"基金持仓{result['stock_fund']} / 持仓趋势{result['stock_fund_trend']}，耗时{result['elapsed_seconds']}s", log_id))
            conn.commit()
        except Exception as e:
            if conn:
                cur = conn.cursor()
...
def update_reports(source: str = 'cninfo', period: str = 'annual', interval: float = 5.0):
    ...
    script = Path(__file__).resolve().parent.parent.parent / 'scripts' / 'download_reports.py'
