...
<style>*{{margin:0;padding:0;box-sizing:border-box}}body{{background:#0d0d1a;color:#ccc;font-family:-apple-system,BlinkMacSystemFont,sans-serif;padding:16px}}h1{{font-size:16px;color:#00d4ff;margin-bottom:4px}}.sub{{font-size:12px;color:#aaa;margin-bottom:12px}}#chart{{width:100%;height:300px;background:#131322;border-radius:10px;border:1px solid #1e1e35;position:relative}}canvas{{width:100%;height:100%}}.legend{{display:flex;gap:16px;margin-top:8px;font-size:11px;color:#ccc;flex-wrap:wrap}}</style>
...
ctx.fillStyle='#bbb';ctx.font='10px sans-serif';ctx.textAlign='right';for(var i=0;i<=4;i++){{ctx.fillText(Math.round(rMax*i/4)+'亿',pad.left-6,pad.top+ch*(1-i/4)+4)}}
ctx.textAlign='left';ctx.fillStyle='#5eead4';for(var i=0;i<=4;i++){{ctx.fillText(Math.round(pm*i/4)+'元',pad.left+cw+6,pad.top+ch*(1-i/4)+4)}}
ctx.fillStyle='#ccc';ctx.font='11px sans-serif';ctx.textAlign='center';for(var i=0;i<n;i++){{ctx.fillText(d.years[i],xs[i],H-pad.bottom+16)}}
}}();</script></html>"""
