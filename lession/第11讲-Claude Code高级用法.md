...
不需要执行任何命令，只需在提问时就告诉 Claude：

> "先写方案，我确认后再编码"

这个简单的提示能让 Claude 先输出分析思路和实现计划，等你确认后再开始写代码。对比直接让 Claude 写代码，可以避免：
- 方向错了全部重来的浪费
- 写了大量不需要的代码
- 中途改需求导致的重复劳动

**推荐流程（五步迭代法）：**
1. **描述需求** — 告诉 Claude 你要什么，让他先出方案，不要写代码
2. **审查方案** — 看 Claude 的理解是否准确，方案是否合理，有没有遗漏
3. **确认执行** — 方案没问题了，说"可以，开始编码"，Claude 才会动手
4. **验证结果** — 代码写完后，让他运行测试、预览效果，确认功能符合需求
5. **迭代改进** — 有问题就指出来让 Claude 改，改完再验证，循环直到满意
